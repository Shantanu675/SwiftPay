...
//       } else {
//         console.warn('Failed to fetch unread notifications:', response.status);
//       }
//     } catch (error) {
...
const Header = ({
  ...
  const [user, setUser] = useState(null);
  const [notifications, setNotifications] = useState([]); // only unread
  const [showNotificationsPanel, setShowNotificationsPanel] = useState(false);
  const [showUserMenu, setShowUserMenu] = useState(false);
  const [showQuickActionsPanel, setShowQuickActionsPanel] = useState(false);
  ...
  useEffect(() => {
    ...
    // Optional: poll for new unread notifications every 20 seconds
  ...
  const markAllAsRead = async () => {
    // Mark each unread notification as read on backend
    for (const notif of notifications) {
      await markNotificationAsRead(notif.id);
    }

    // Small delay to let backend process → then refresh list
    setTimeout(() => {
      fetchNotifications();
      setShowNotificationsPanel(false);
    }, 400);
  ...
  // Quick Actions Data (unchanged)
  const quickActions = [
    { icon: '🏠', label: 'Dashboard', path: '/home', description: 'Go to main dashboard' },
    { icon: '💸', label: 'Send Money', path: '/send-money', description: 'Transfer to friends & family' },
    { icon: '💰', label: 'Add Funds', path: '/add-funds', description: 'Top up your wallet' },
    { icon: '📊', label: 'Transactions', path: '/transactions', description: 'View transaction history' },
    { icon: '🎁', label: 'Rewards', path: '/rewards', description: 'Claim your rewards' },
    { icon: '👤', label: 'Profile', path: '/profile', description: 'Manage your account' }
  ];
  ...
  // Replace your current time display with this helper
